...
{	
	...
	for (int i=0; i<10; i++)
	{
		i2cdevReadByte(0, BMP280_I2C_ADDR, BMP280_CHIP_ID, &id);//读取ID
	...
	{
		i2cdevRead(0, BMP280_I2C_ADDR, BMP280_TEMPERATURE_CALIB_DIG_T1_LSB_REG, 24, (u8*)&bmp280Cal);//读取校准数据
		i2cdevWriteByte(0, BMP280_I2C_ADDR, BMP280_CTRL_MEAS_REG, BMP280_MODE);//设置过采样率和工作模式
		i2cdevWriteByte(0, BMP280_I2C_ADDR, BMP280_CONFIG_REG, BMP280_FILTER);//配置IIR滤波
...
//	if(status & 0x08)
	{	
		i2cdevRead(0, BMP280_I2C_ADDR, BMP280_PRESSURE_MSB_REG, BMP280_DATA_FRAME_SIZE, data);
