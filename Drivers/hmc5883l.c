...
bool hmc5883lInit(void)
{
	...
	for (int i=0; i<10; i++)
	{
		i2cdevReadByte(0, 0x1E, 0x0A, &id);//读取ID
	...
	if (id == 'H')
	{
		i2cdevWriteByte(0, MAG_ADDRESS, HMC58X3_R_CONFA, 0x78);// Configuration Register A  -- 0 11 100 00  num samples: 8 ; output rate: 75Hz ; normal measurement mode
		delay_ms(5);
		i2cdevWriteByte(0, MAG_ADDRESS, HMC58X3_R_CONFB, 0x20);// Configuration Register B  -- 001 00000    configuration gain 1.3Ga
		delay_ms(5);
		i2cdevWriteByte(0, MAG_ADDRESS, HMC58X3_R_MODE, 0x00); // Mode register             -- 000000 00    continuous Conversion Mode
		delay_ms(100);
		isInit = true;
...
{
    uint8_t buf[6];
	bool ack = i2cdevRead(0, MAG_ADDRESS, MAG_DATA_REGISTER, 6, buf);
